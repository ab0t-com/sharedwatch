...
You only need to know FIVE commands:
  ...
  sharedwatch overview --format json
    → L1: per-root counts, last_event_at, mode.
    [FUTURE — fall back to:
  ...
  sharedwatch events list \
    ...
    --fields id,type,rel_path,actor,session,observed_at \
    --format jsonl
...
(1) PATTERN: initial-orientation
    ...
    Steps:
      ...
      c. `sharedwatch events stats --root <X> --since 24h
         --format json`  [FUTURE]
         or: `sharedwatch sql "SELECT type, COUNT(*) FROM events
...
(2) PATTERN: peer-handoff-check
    ...
    Steps:
      ...
      b. If a recent event from a non-self `actor` exists,
         ...
         (`sharedwatch status --actors` [FUTURE]) or the timestamp.
...
(7) PATTERN: don't-step-on-toes (advisory, no enforcement)
    ...
    Steps:
      ...
      c. (Once leases ship, acquire one:
         `sharedwatch lease grant <path> --ttl 5m
          --actor <you>` [FUTURE].)
...
- DOGFOOD pattern: see `test_dogfood.md` for canonical scenarios.
...
- **The `[FUTURE]` markers are deliberate.** Some referenced commands (`overview`, `events stats`, `lease`, `status --actors`) aren't built yet. The prompt names them with a fallback so the same prompt works pre- and post-implementation.
...
- `sharedwatch/docs/SCHEMA_CONTRACTS.md` — authoritative column inventory.
- `sharedwatch/docs/APPLICATION_FLOW.md` — the pipeline picture.
- `test_dogfood.md` (top-level) — realistic scenarios to learn from.
- `sharedwatch-multi-agent-discussion-20260522.md` — the *why* behind the design (only useful for agents that reason about design).
