...
func (s *Store) QueryEvents(ctx context.Context, f EventFilter) ([]events.Event, error) {
	...
	q := fmt.Sprintf(`SELECT id, type, path, rel_path, old_path, source, status, retry_count, observed_at, file_size, mtime, content_hash, coalesced_into, payload_json, producer_id
