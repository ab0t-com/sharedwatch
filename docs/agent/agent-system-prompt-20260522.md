...
- **`--all` on coordination-list commands is a TTL-expiry filter, not a soft-delete filter** — applies to **both** `lease list --all` and `intent list --all` (scenarios 31 and 33). Released leases and revoked intents are hard-deleted, not retained; `--all` only surfaces entries that lived out their TTL without explicit teardown. For leases, the audit trail lives in the events journal (every grant/release emits an event). For intents, **no events are emitted yet** for declare/revoke — poll `intent list --json` if you need to track lifecycle (open design question, see `docs/design/intent-events-discussion-20260524.md`).
...
- **`test emit` against a (actor, path) pair already emitted within the 5s coalesce window** returns a fresh `evt_id` in its CLI output, but the underlying row is COALESCED into the prior event — `events list` will never surface that id. Don't record the returned id and reference it later; if you need to know your emit landed as a fresh row, query by path immediately after the emit. Surfaced by scenario 34.
