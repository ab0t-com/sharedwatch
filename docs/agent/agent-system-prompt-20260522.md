...
- **`config show --json` uses Go-style CapitalCase keys** (`WatchPath`, `DBPath`, `CoalesceWindow`) and encodes durations as raw nanoseconds (`5000000000` = 5s). Every other JSON endpoint uses snake_case. Special-case `config show` when scripting. Surfaced by scenario 36. **Fixed in v0.0.10 (SW-AGENT-25)** — now uses snake_case keys + duration strings (`"5s"`).
- **Cursor filter-change is SILENT — no warning emitted.** Reusing a named cursor with a different `--type`, `--path-glob`, `--payload-key/--payload-value`, or other filter scope silently skips events past the prior position that didn't match the prior filter. Empirically verified by dogfood scenario 40 (the binary does NOT warn, despite older docs that suggested it did). Always use a NEW cursor name when you change filter scope (e.g. `<actor>-<task>-<scope-hash>`). Open ticket: SW-AGENT-28 (see `docs/design/cursor-filter-change-discussion-20260524.md`).
- **Hook meta-events (`hook.completed` / `hook.failed`) live in the journal.** v0.1.0+ daemons running with `--on-digest <cmd>` (SW-AGENT-29) emit one meta-event per hook run, source=`hook`. Query with `events list --type hook.completed --type hook.failed --json`; payload carries `digest_id`, `exit_code`, `duration_ms`, `reason` (`success`/`timeout`/`nonzero_exit`/`spawn_error`), `stdout_path`/`stderr_path`. Sidecar files at `<data_dir>/hooks/<digest_id>.{out,err}` hold the full stdout/stderr. Useful for: confirming an external integration fired, diagnosing why a webhook/script didn't post, auditing hook activity over time. Hook meta-events are stamped `source=hook` so file-event queries like `events list --source watcher,reconciler,test` exclude hook noise.
