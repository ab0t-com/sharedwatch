...
- **`sharedwatch lease list --all` shows expired leases but NOT released leases.** Surfaced by dogfood scenario 31. The help text "include expired leases" is accurate but easy to misread as "everything I ever did". Released leases are deleted outright; only leases that lived out their TTL are retained for `--all` to surface. If you need a full audit trail of granted-then-released leases, query the `events` journal (every lease grant/release emits an event) rather than `lease list --all`.
