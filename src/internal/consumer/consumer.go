...
// dominantRoot returns the watch_root that should label the produced digest:
// the single shared root when all events agree, "mixed" when the batch spans
// roots, "" when the batch is purely legacy single-root events.
func dominantRoot(evs []events.Event) string {
	if len(evs) == 0 {
		return ""
	}
	seen := evs[0].WatchRoot
	for _, e := range evs[1:] {
		if e.WatchRoot != seen {
			return "mixed"
		}
